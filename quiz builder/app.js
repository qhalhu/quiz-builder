...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Status banner
  const banner = $("#jsBanner");
  if (banner) banner.textContent = "✅ JavaScript is running (PWA-ready).";
  ...
  // Storage (local, per-origin)
  const STORAGE_KEY = "quizBuilder.v8.emoji.pwa.quizzes";
  let memoryStore = [];
  function safeLoad(){
    try{ const raw = localStorage.getItem(STORAGE_KEY); return raw ? JSON.parse(raw) : []; }
    catch(e){ showError("localStorage load failed: "+e); return memoryStore; }
  }
  function safeSave(arr){
    try{ localStorage.setItem(STORAGE_KEY, JSON.stringify(arr)); }
    catch(e){ showError("localStorage save failed: "+e); memoryStore = arr; }
  }

  // State
  let quizzes = safeLoad();
  let activeId = quizzes[0]?.id || null;  // currently selected in sidebar
  let parsedDraft = null;                  // last parsed builder draft
  let session = null;

  function toast(msg){ if (banner) banner.textContent = "ℹ️ " + msg; }
  ...
  function renderQuizList(){
    ...
    quizzes.forEach(q=>{
      ...
      play.onclick = ()=>{ activeId=q.id; switchTab('player'); startSession(); };

      const edit = document.createElement('button'); edit.className='btn'; edit.textContent='Edit';
      edit.onclick = ()=>{ activeId=q.id; switchTab('builder'); loadQuizIntoBuilder(q); };

      const exp = document.createElement('button'); exp.className='btn'; exp.textContent='Export';
      exp.title = "Download this quiz as JSON";
      exp.onclick = ()=>exportSingleQuiz(q);

      const del = document.createElement('button'); del.className='btn danger'; del.textContent='Delete';
      del.title = "Remove this saved quiz";
      del.onclick = ()=>{
        if (confirm(`Delete quiz "${q.name}"? This cannot be undone.`)) {
    ...
    $("#activeQuizLabel").textContent = activeQuiz ? "Active: "+activeQuiz.name : "No quiz selected";
  }
  ...
  function loadQuizIntoBuilder(q){
    ...
    const blocks = q.questions.map((qq,i)=>{
      ...
      qq.choices.forEach(c=>{ lines.push((c.correct?'✅ ':'❎ ')+qqChoiceText(c)); });
  ...
  const qqChoiceText = c => c.text; // for future-proofing

  // Parse ✅/❎ or legacy +/-; optional A) ... Answer: A,C style
  function parseChoiceLine(ln){
    const s = ln.trim();
    if (/^[✅❎]/.test(s)){
      const correct = s.startsWith("✅");
      return { text: s.replace(/^[✅❎]\s*/,""), correct };
    }
    if (/^[\+\-]/.test(s)){
      const correct = s.startsWith("+");
      return { text: s.replace(/^[\+\-]\s*/,""), correct };
  ...
  function parseRaw(){
    ...
    const blocks = raw.replace(/^#\s*Quiz:.+$/gmi,"").trim().split(/\n\s*\n+/);
    const questions = [];
    ...
    // If editing existing and title unchanged, keep ID to overwrite
  ...
  function exportSingleQuiz(q){
    saveAsJSON((q.name.replace(/[^a-z0-9\-]+/gi,'_')||'quiz')+'.json', q);
  }
  function exportAllQuizzes(){
    const payload = { exportedAt: new Date().toISOString(), quizzes };
    saveAsJSON('quizzes_export.json', payload);
  }
  ...
  $("#parseBtn").onclick = ()=>{
    ...
    if (draft){
      parsedDraft = draft;
      renderPreview(draft);
      toast("Parsed "+draft.questions.length+" question(s)");
    }
  };
  ...
  // Save with overwrite + duplicate-title warning
  $("#saveQuizBtn").onclick = ()=>{
    ...
    const toSave = {
      id: existsById ? parsedDraft.id : uid(),
      name: title,
      options: parsedDraft.options,
      questions: parsedDraft.questions
    };
    if (existsById){
      quizzes = quizzes.map(q=>q.id===toSave.id ? toSave : q);
    } else {
      quizzes.unshift(toSave);
    }
    activeId = toSave.id;
  ...
  $("#importFile").addEventListener('change', (e)=>{
    ...
    reader.onload = ()=>{
      try{
        ...
        const arr = Array.isArray(obj)? obj : (obj.quizzes || [obj]); // supports full export
        arr.forEach(q=>{
          q.id = q.id || uid();
